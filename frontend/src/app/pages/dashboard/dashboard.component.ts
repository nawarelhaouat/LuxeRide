...
export class DashboardComponent implements OnInit {
  ...
  @ViewChild('utilisationCanvas')
  utilisationCanvas!: ElementRef<HTMLCanvasElement>;

  @ViewChild('revenusCanvas')
  revenusCanvas!: ElementRef<HTMLCanvasElement>;
  ...
  private createReservationsLineChart(labels: string[], values: number[]): void {
    const canvas = this.reservationsCanvas?.nativeElement;
    if (!canvas) return;
  ...
  private createUtilisationPieChart(labels: string[], values: number[]): void {
    const canvas = this.utilisationCanvas?.nativeElement;
    if (!canvas) return;
  ...
  private createRevenusBarChart(labels: string[], values: number[]): void {
    const canvas = this.revenusCanvas?.nativeElement;
    if (!canvas) return;
